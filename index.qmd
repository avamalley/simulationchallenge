...
**Key Concept:** This simple coin flip game represents a single-period game with additive winnings. Unlike the investment game which shows how wealth compounds over time with multiplicative changes ($W_t = 1.5 \times W_{t-1}$ or $W_t = 0.6 \times W_{t-1}$), this simple game shows fixed winnings ($W = +100$ or $W = -100$) based on a single coin flip outcome.
...
# Create simulation data using base R
sim_data <- data.frame(
  sim_num = 1:n_sims,
...
# Calculate winnings
sim_data$winnings <- ifelse(sim_data$coin_flip == 1, 100, -100)
...
**Key Concept:** This time-series simulation models sequential balance evolution over multiple periods. Each period's balance depends on the previous period's balance plus the current coin flip outcome. Unlike the simple coin flip game which shows independent winnings per flip, this model shows cumulative balance changes where $B_t = B_{t-1} + \Delta_t$ and $\Delta_t = +100$ or $-100$ based on the coin flip.
...
# Create time series plot using base R
plot(time_series_data$period, time_series_data$balance, 
     type = "b", col = "cadetblue", lwd = 2, pch = 19,
     main = "Account Balance Over Time\nSingle Simulation Path",
     xlab = "Period", ylab = "Account Balance ($)",
     ylim = c(0, max(time_series_data$balance) * 1.1))
grid()
...
hist(distribution_data$final_balance, 
     ...
     col = "plum", 
     border = "black",
     main = paste("Distribution of Final Account Balances\n100 Simulations,", n_periods, "Periods Each"),
     xlab = "Final Balance ($)",
     ylab = "Frequency")
...
cat("Mean balance: $", round(mean_balance, 2), "\n", sep = "")
cat("Median balance: $", round(median_balance, 2), "\n", sep = "")
